...
echo "[+] Run 'subfinder'..."
$GOPATH/bin/subfinder -d $DOMAIN -silent > $RAW
echo "[+] Run 'assetfinder'..."
$GOPATH/bin/assetfinder --subs-only $DOMAIN >> $RAW
...
echo "[+] Run 'httpx'..."
cat $NEW | $GOPATH/bin/httpx -mc 200,301,302 -silent -status-code -title -tech-detect -no-color > "$FOLDER/new_live_hosts.txt"
